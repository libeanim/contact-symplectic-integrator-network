...
def TRAIN(env, model, name, learning_rate=1e-2, loss='mse', pos_only=False, initialise=True, learn_friction=False):
    ...
    for ep in range(100):
        for b, (x_b, y_b) in enumerate(dataset):
            m_out = m.predict_on_batch(x_b)
            print(x_b)
            print(m_out)
            loss = m.train_on_batch(x_b, y_b)
            print(loss)
            if np.any(np.isnan(m_out.numpy())):
                exit()
            #exit()
    """
...
def PREDICT(env, model):
    ...
    ----------
        ...
        model:  Trained model
    """
    return model.predict_forward(tf.convert_to_tensor([[env.trajectory[0, :-1]]], np.float32), env.dt, env.trajectory.shape[0])[0]